      ...
      if(( time - Date.now())>0){    
        bookingsStr=bookingsStr + ` <div class="p-4 rounded shadow-sm bg-light my-3">
          <p class="font-italic mb-0"><span id="toBeTherapyDate" class="mx-2">${time.getFullYear()}/${time.getMonth()+1}/${time.getDate()}</span><span
           ...
           <p class="font-italic my-2"><div class="mx-2">失眠：一周${booking.insomnia}天</div> <div class="mx-2">抑鬱：一周${booking.depressed}天</div> <div class="mx-2">焦慮：一周${booking.panic}天</div> <div class="mx-2">其他症狀：一周${booking.other_symptoms}天</div></p>
        </div>`
